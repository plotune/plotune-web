...
async function showBootBanner() {
  ...
  const projectFolder = String(directory).split(/[\\/]/).filter(Boolean).pop() || 'demo-project';
  const modelLabel = 'Opus 4.7 · API Usage Billing';
  const bannerText = [
  ...
  add('boot', `
    <div class="claude-welcome">
      <div class="boot-title">Claude Code v2.1.101</div>
      <div class="boot-left">
        <div class="boot-welcome">Welcome back!</div>
        <div class="boot-logo">▐▛███▜▌<br>▝▜█████▛▘<br>▘▘ ▝▝</div>
        <div class="boot-model">${esc(modelLabel)}</div>
        <div class="boot-path">${esc(projectFolder)}</div>
      </div>
      <div class="boot-right">
        <div class="boot-section-title">Tips for getting started</div>
        <div class="boot-line">Run /init to create a CLAUDE.md file with instructions for Claude</div>
        <div class="boot-line">Note: You have launched claude in this project folder. For the best experience, keep it scoped here …</div>
        <div class="boot-rule"></div>
        <div class="boot-section-title">Recent activity</div>
        <div class="boot-muted">No recent activity</div>
      </div>
    </div>
  `);
  add('shortcut-hint', '? for shortcuts');
  await window.pauseForRead(bannerText, { baseMs: 900, perCharMs: 1.1, capMs: 1800 });
}
