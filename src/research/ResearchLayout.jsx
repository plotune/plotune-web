...
const ResearchLayout = ({ children }) => {
  ...
  const copyFeed = async () => {
    if (navigator.clipboard) await navigator.clipboard.writeText(rssUrl);
    setCopied(true);
    window.setTimeout(() => setCopied(false), 1800);
  };
  return <div className="research-site"><header className="research-header"><a className="research-brand" href="https://plotune.net" aria-label="Return to Plotune"><span>Plotune</span><strong>Research</strong></a><button className="research-menu" onClick={() => setOpen(!open)} aria-expanded={open}>Menu</button><nav className={open ? 'is-open' : ''}>{nav.map(([to, label]) => <Link key={to} to={to} className={location.pathname === to ? 'active' : ''}>{label}</Link>)}<div className="rss-control"><button className="rss-button" onClick={() => setRssOpen(!rssOpen)} aria-label="RSS options" aria-expanded={rssOpen}><RssIcon /></button>{rssOpen && <div className="rss-popover"><strong>Follow this research</strong><a href={`https://feedly.com/i/subscription/feed/${encodeURIComponent(rssUrl)}`} target="_blank" rel="noreferrer">Open in Feedly</a><button onClick={copyFeed}>{copied ? 'Feed link copied' : 'Copy feed link'}</button></div>}</div><a className="research-login" href="https://plotune.net/login">Log in</a><a className="research-register" href="https://plotune.net/register">Register</a></nav></header><main>{children}</main><footer className="research-footer"><span>(c) {new Date().getFullYear()} Plotune Research</span><a href="/research/rss.xml">RSS feed</a></footer></div>;
};
