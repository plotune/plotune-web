...
This is why AI HIL adoption looks so different from AI adoption elsewhere in the test stack. Teams are comfortable letting an agent draft test cases or triage a failing CI run — none of that touches hardware. Hardware in the loop is different because the cost of being wrong isn't a bad summary, it's a command that reaches a real bus, a real actuator, or a real bench someone else depends on.

So the problem isn't "can an agent run a HIL sequence" — it can. It's "what's the smallest, most provable set of things the agent is ever allowed to do, and how do we know it never did anything outside that set." Without an answer, every run comes down to trusting that the output "looks right," which isn't a boundary you can point to — it's a hope.
...
One concrete shape of this is a containerized simulation running on virtual CAN. The controller under test runs as a staged container, connected to a virtual CAN bus rather than a physical one. A policy gateway sits between the virtual bus and real CAN, so only frames that were explicitly approved are allowed to cross from simulation into anything that touches real hardware. When that boundary is crossed — when a frame does move from virtual to real — the crossing itself gets captured as proof: not a claim that it happened correctly, but an artifact that shows exactly what crossed, when, and under what approval.
