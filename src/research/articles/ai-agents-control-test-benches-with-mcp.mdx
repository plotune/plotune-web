...
<FlowDiagram title="A CAN operation through MCP" steps={["Acquire the interface", "Send an approved message", "Wait for the signal", "Release the interface"]} />
...
<StatFigure title="Bounded by design" stats={[["Tools", "Bounded set"], ["Raw shell access", "None"], ["Approval", "Operational"], ["Audit", "Per run"]]} />
