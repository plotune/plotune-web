...
import ScatterMetricChart from '../components/research/ScatterMetricChart';
...
const ResearchOverview = () => {
  ...
  const taskGroup = params.get('task') || 'All tasks';
  const metric = params.get('metric') || 'performance';
  const page = Math.max(1, Number(params.get('page')) || 1);
  const landscapeMetric = landscapeMetrics.includes(params.get('landscape')) ? params.get('landscape') : 'cost';
  const view = useMemo(() => buildBenchmarkView({ taskGroup, metric, sort: metric }), [taskGroup, metric]);
  const resultsView = useMemo(() => buildBenchmarkView({ taskGroup, sort: 'performance' }), [taskGroup]);
